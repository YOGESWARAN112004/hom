...
export class DatabaseStorage implements IStorage {
  ...
  async calculateOrderCommission(orderId: string): Promise<number> {
    ...
    const affiliate = await this.getAffiliate(order.affiliateId);
    if (!affiliate || affiliate.status !== 'approved' || !affiliate.isActive) {
      return 0;
    }
    ...
    for (const item of items) {
      // Get product to check for product-specific commission rate
      const product = await this.getProduct(item.productId);
      
      // Use product-specific rate if available, otherwise use affiliate default rate
      const commissionRate = product?.affiliateCommissionRate 
        ? parseFloat(product.affiliateCommissionRate)
        : parseFloat(affiliate.commissionRate);

      // Calculate commission on item subtotal (unitPrice × quantity)
  ...
  async reverseAffiliateCommission(affiliateId: string, amount: number): Promise<void> {
    await db.update(affiliates)
      .set({
        totalEarnings: sql`${affiliates.totalEarnings} - ${amount}`,
        pendingEarnings: sql`${affiliates.pendingEarnings} - ${amount}`,
    ...
    if (items.length > 0) {
      ...
      for (const item of items) {
        if (item.variantId) {
          ...
          if (variant) {
            await this.updateVariantStock(item.variantId, Math.max(0, variant.stock - item.quantity));
          }
        } else {
  ...
  async updateOrderStatus(id: string, status: string): Promise<void> {
    ...
    if (status === 'shipped') {
      ...
      // Calculate and add commission when order is delivered
      // Only if transitioning TO delivered (not if already delivered) and payment is confirmed
      if (previousStatus !== 'delivered' && !currentOrder.deliveredAt && currentOrder.affiliateId && currentOrder.paymentStatus === 'paid') {
        const commission = await this.calculateOrderCommission(id);
        if (commission > 0) {
          await this.updateAffiliateEarnings(currentOrder.affiliateId, commission);
          console.log(`Commission of ₹${commission} added for affiliate ${currentOrder.affiliateId} on order ${id}`);
        }
      }
    } else if ((status === 'cancelled' || status === 'refunded') && previousStatus === 'delivered') {
      // Reverse commission if order was delivered and is now cancelled/refunded
      // Only reverse if order was actually delivered (has deliveredAt timestamp)
      if (currentOrder.affiliateId && currentOrder.deliveredAt) {
        const commission = await this.calculateOrderCommission(id);
        if (commission > 0) {
          await this.reverseAffiliateCommission(currentOrder.affiliateId, commission);
          console.log(`Commission of ₹${commission} reversed for affiliate ${currentOrder.affiliateId} on order ${id} due to ${status}`);
        }
      }
    }
  ...
  async getCartItems(userId: string): Promise<(CartItem & { product: Product; variant: ProductVariant | null })[]> {
    ...
    return enrichedItems.filter((item) => item.product !== undefined);
  }
  ...
  async addToCart(item: InsertCartItem): Promise<CartItem> {
    ...
    if (existing) {
      ...
      return { ...existing, quantity: existing.quantity + (item.quantity || 1) };
    }
  ...
  async getWishlistItems(userId: string): Promise<(WishlistItem & { product: Product })[]> {
    ...
    const enrichedItems = await Promise.all(items.map(async (item: WishlistItem) => {
      const product = await this.getProduct(item.productId);
      return {
        ...item,
        product: product!,
    ...
    return enrichedItems.filter((item: { product: Product | undefined }) => item.product);
  }
  ...
  async getPopupAnnouncements(): Promise<Announcement[]> {
    ...
    return await db.select().from(announcements)
      .where(and(
        eq(announcements.isActive, true),
        eq(announcements.showAsPopup, true),
        or(
          sql`${announcements.startDate} IS NULL`,
          lte(announcements.startDate, now)
        ),
  ...
  async getAnalytics(startDate?: Date, endDate?: Date): Promise<{
    ...
    const lowStockProducts = await db
      ...
      .where(and(
        eq(products.isActive, true),
        lte(products.stock, sql`coalesce(${products.lowStockThreshold}, 5)`)
      ));
    ...
    const salesByDay = await db
      .select({
        ...
        sales: sql<number>`coalesce(sum(${orders.totalAmount}), 0)`.as('sales'),
        orders: sql<number>`count(*)`.as('orders'),
      })
      .from(orders)
      .where(and(
        eq(orders.paymentStatus, 'paid'),
        gte(orders.createdAt, new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000))
      ))
    ...
    const topProducts = await db
      .select({
        ...
        sales: sql<number>`sum(${orderItems.quantity})`.as('sales'),
  ...
  async getBlogs(publishedOnly: boolean = false): Promise<Blog[]> {
    if (publishedOnly) {
      return await db.select()
        .from(blogs)
        .where(and(eq(blogs.isPublished, true)))
        .orderBy(desc(blogs.publishedAt || blogs.createdAt));
    }
